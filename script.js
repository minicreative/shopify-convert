...
var getShopifyRow = function (format) {
	...
	if (format == 'header') return shopifyRow;
};
...
function startShopify() {
    ...
    var shopifyCSV = document.getElementById("ShopifyUpload").files[0];

    // Parse and send to formatter
    Papa.parse(productsCSV, {
        header: true,
    	complete: function(products) {
			Papa.parse(shopifyCSV, {
				complete: function(shopify) {
					createShopifyCSV(shopify, products);
				}
			});
    	}
    });
...
function createShopifyCSV(shopify, products) {

    // Initialize Shopify headers
    var headers = shopify.data[0];
    var len = headers.length;

    // Get products array
    var prods = products.data;

    // Get parent products
    var parents = {};
    for (var i in prods)
        if (!prods[i].ischildofproductcode || prods[i].ischildofproductcode.length < 2)
            parents[prods[i].productcode] = prods[i];

    // Push children into parents
    for (var i in prods) {
        var code = prods[i].ischildofproductcode;
        if (code && parents[code]) {
            if (!parents[code].children) parents[code].children = [];
            parents[code].children.push(prods[i]);
        }
    }

    // Push headers into output
    output.push(headers);
